        ...
        if (UserInd != userV):
            for i in range(1, count_col):
                if (data[" Movie " + str(i)][userV] != -1 and
                        (context_day[" Movie " + str(i)][userV] == ' Sun' or context_day[" Movie " + str(i)][
                            userV] == ' Sat')
...
# вариант 24
# список стран рождения актеров фильма, отсортированный по популяции, с
# указанием имени актера
filmIndex = int(film[-2:]) - 1
# print("filmIndex = "+filmIndex)
names = pd.read_csv('movie_names.csv', names=['Movie', 'Name'])
#print(names)
FilmName = names["Name"][filmIndex]
...
print("Description - " + res.json()['search'][0]['description'])

print("Id - " + res.json()['search'][0]['id'])

sparql = SPARQLWrapper("https://query.wikidata.org/sparql")

sparql_query = """

#SELECT ?actor ?actorLabel ?actorNameLabel ?countryOfBirthLabel ?populationLabel
SELECT ?actor ?actorNameLabel ?countryOfBirthLabel 
WHERE {
  ?film wdt:P31 wd:Q11424.
  ?film wdt:P161 ?actor.
  ?actor wdt:P19 ?townOfBirth.
  ?townOfBirth wdt:P17 ?countryOfBirth.
  ?countryOfBirth wdt:P1082 ?population.
  ?actor wdt:P735 ?actorName
               
  FILTER(?film = wd:Q171669)
...
print(results_df[['actorNameLabel.value', 'countryOfBirthLabel.value']])
